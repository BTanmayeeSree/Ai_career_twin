...
function initializeResumeUpload() {
    ...
    uploadZone.addEventListener('click', (e) => {
        // Prevent double dialogs when clicking the internal "Browse Files" label
        const isLabelClick = e.target && (e.target.tagName === 'LABEL' || e.target.closest('label'));
...
function computeLevelsFromResume(data) {
    ...
    const interviewReadiness = calculateInterviewReadinessFromLevels({
        technical,
        communication,
        projectMaturity
    });
...
function calculateInterviewReadinessFromLevels(levels) {
...
// ============================================
function initializeLogoHandlers() {
    ...
    if (btn && input) {
        ...
        input.addEventListener('change', (e) => {
            ...
            const reader = new FileReader();
            reader.onload = (ev) => {
                localStorage.setItem('aiCareerTwin_logo', ev.target.result);
                updateLogoDisplay();
...
function updateLogoDisplay() {
    const dataUrl = localStorage.getItem('aiCareerTwin_logo');
    const imgNav = document.getElementById('appLogo');
    const imgFooter = document.getElementById('appLogoFooter');
    const navIcon = document.getElementById('navBrandIcon');
    const footerIcon = document.getElementById('footerBrandIcon');
    if (!imgNav || !imgFooter) return;
    const defaultSrc = 'images/logo.svg';
    const src = dataUrl || defaultSrc;
    imgNav.src = src;
    imgFooter.src = src;
    imgNav.style.display = 'inline-block';
    imgFooter.style.display = 'inline-block';
...
function exportToPDF() {
    ...
    if (!jsPDF) {
        showNotification('PDF library not available.');
        return;
    }
    ...
    const opt = {
        ...
        callback: function (doc) {
            doc.save('AI-Career-Twin-Results.pdf');
        }
    };
    pdf.html(container, opt);
}

function exportToPNG() {
    const container = document.getElementById('resultsContainer') || document.body;
    if (!container || !window.html2canvas) {
        showNotification('PNG export unavailable.');
        return;
    }
    html2canvas(container, { scale: 2, useCORS: true }).then(canvas => {
        const link = document.createElement('a');
        link.download = 'AI-Career-Twin-Results.png';
        link.href = canvas.toDataURL('image/png');
        link.click();
        showNotification('PNG exported');
    }).catch(() => {
        showNotification('Failed to export PNG');
    });
}
...
function calculateAndDisplayScenarios() {
    ...
    const baselineParams = {
        ...
        consistency: careerTwin.practiceConsistency || 7,
        communication: careerTwin.communicationSkill || 6,
...
function recalculateScenarios() {
    const baselineParams = {
        ...
        consistency: parseFloat(document.getElementById('scenarioPractice')?.value || 7),
        communication: parseFloat(document.getElementById('scenarioComm')?.value || 6),
